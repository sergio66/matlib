...
figure(5); scatter_coast(p.rlon,p.rlat,10,tcc4A); colormap jet; title('METHOD 4A'); caxis([0 1])
figure(6); scatter_coast(p.rlon,p.rlat,10,tcc4B); colormap jet; title('METHOD 4B'); caxis([0 1])
figure(6); scatter_coast(p.rlon,p.rlat,10,p.tcc); colormap jet; title('ORIG'); caxis([0 1])

     [Y,I] = sort(p.tcc);
     figure(6); plot(p.tcc(I),tcc1(I),'b',p.tcc(I),tcc2(I),'g',p.tcc(I),tcc3A(I),'r',p.tcc(I),tcc3B(I),'m',p.tcc(I),tcc4A(I),'k',p.tcc(I),tcc4B(I),'y')
     ...
     corr4A = linearcorrelation(p.tcc,tcc4A);
     corr4B = nanlinearcorrelation(p.tcc,tcc4B);
     %[corr1 corr2 corr3A corr3B corr4A corr4B]
     ...
     ratio4A(ii) = nanmean((tcc4A + eps)./(p.tcc + eps)); stdratio4A(ii) = nanstd((tcc4A + eps)./(p.tcc + eps));
     ratio4B(ii) = nanmean((tcc4B + eps)./(p.tcc + eps)); stdratio4B(ii) = nanstd((tcc4B + eps)./(p.tcc + eps));
     ...
     bias4A(ii) = nanmean(tcc4A-p.tcc); stdbias4A(ii) = nanstd(tcc4A-p.tcc);
     bias4B(ii) = nanmean(tcc4B-p.tcc); stdbias4B(ii) = nanstd(tcc4B-p.tcc);
     ...
     ratio4A(ii) = NaN;     stdratio4A(ii) = NaN;
     ratio4B(ii) = NaN;     stdratio4B(ii) = NaN;

     bias1(ii) = NaN;      stdbias1(ii) = NaN;
     bias2(ii) = NaN;      stdbias2(ii) = NaN;
     bias3A(ii) = NaN;     stdbias3A(ii) = NaN;
     bias3B(ii) = NaN;     stdbias3B(ii) = NaN;
     bias4A(ii) = NaN;     stdbias4A(ii) = NaN;
     bias4B(ii) = NaN;     stdbias4B(ii) = NaN;
  ...
  figure(6); plot(1:ii,allcorr1,'b',1:ii,allcorr2,'g',1:ii,allcorr3A,'r',1:ii,allcorr3B,'m',1:ii,allcorr4A,'k',1:ii,allcorr4B,'y')
     ylabel('corr <tccX,p.tcc>')
  figure(7);
subplot(211); plot(1:ii,ratio1,'b',1:ii,ratio2,'g',1:ii,ratio3A,'r',1:ii,ratio3B,'m',1:ii,ratio4A,'k',1:ii,ratio4B,'y')
       ylabel('<tccX/p.tcc>')
       ax = axis; axis([ax(1) ax(2)  0.75 1.25])
       subplot(212); plot(1:ii,stdratio1,'b',1:ii,stdratio2,'g',1:ii,stdratio3A,'r',1:ii,stdratio3B,'m',1:ii,stdratio4A,'k',1:ii,stdratio4B,'y')
       ylabel('std <tccX/p.tcc>')
       ax = axis; axis([ax(1) ax(2)  0 0.5])
figure(8);
subplot(211); plot(1:ii,-bias1,'b',1:ii,-bias2,'g',1:ii,-bias3A,'r',1:ii,-bias3B,'m',1:ii,-bias4A,'k',1:ii,-bias4B,'y')
     ylabel('<p.tcc-tccX>')
subplot(212); plot(1:ii,stdbias1,'b',1:ii,stdbias2,'g',1:ii,stdbias3A,'r',1:ii,stdbias3B,'m',1:ii,stdbias4A,'k',1:ii,stdbias4B,'y')
