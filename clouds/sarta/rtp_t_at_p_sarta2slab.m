function [t] = rtp_t_at_p_sarta2slab(p, head, prof, lextrap);

% copied from /umbc/xfs2/strow/asl/matlib/rtptools/rtp_t_at_p.m
%
% function [t] = rtp_t_at_p_sarta2slab(p, head, prof, lextrap);
%
...
if (length(d) ~= 2 | min(d) ~= 1)
   disp('Error in rtp_t_at_p_sarta2slab: p must be a [1 x 1] scaler or [1 x n] array')
   return
end
...
if (isfield(head,'ptype'))
   ...
   disp('Error in rtp_t_at_p_sarta2slab: head is missing required field ptype')
   return
end
...
if (isfield(prof,'nlevs'))
   ...
   disp('Error in rtp_t_at_p_sarta2slab: prof is missing required field nlevs')
   return
end
...
if (isfield(prof,'ptemp'))
   ...
   disp('Error in rtp_t_at_p_sarta2slab: prof is missing required field ptemp')
   return
end
...
if(ptype == 0 | ptype == 2)
   ...
   if (isfield(prof,'plevs'))
      ...
      disp('Error in rtp_t_at_p_sarta2slab: prof is missing required field plevs')
      return
   end
else
   pl = prof.plevs;
