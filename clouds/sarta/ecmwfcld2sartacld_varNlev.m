...
%%%% replace nlev with profX.nlevs(ii)
%%%% replace nlev with profX.nlevs(ii)
...
for iiiiA = 1:length(iiii)
  ...
  xnlevs = 1:profX.nlevs(ii);

  %%% this has all beeen superseded by stuff below *****************
  iSmoothX = iSmooth;
  blahW = profX.clwc(xnlevs,ii)/(max(profX.clwc(xnlevs,ii))+1e-16);
  blahI = profX.ciwc(xnlevs,ii)/(max(profX.ciwc(xnlevs,ii))+1e-16);
  numI = length(find(blahI > rGaussianCutoff*blahI));
  numW = length(find(blahW > rGaussianCutoff*blahW));
  iDo = -1;
  if profX.nlevs(iiii) > 20
    if numI > 3 & numW > 3
      iSmoothX = iSmooth;
      iDo = +1;
  ...
  if iDo > 0
    % lots of points
    [shiftedx,shiftedy,plevs]    = smooth1aa(xnlevs,profX.plevs(xnlevs,ii)',iSmoothX);
    [shiftedx,shiftedy,watercld] = smooth1aa(xnlevs,profX.clwc(xnlevs,ii)' ,iSmoothX);
    [shiftedx,shiftedy,icecld]   = smooth1aa(xnlevs,profX.ciwc(xnlevs,ii)' ,iSmoothX);

    %npoly = 1;    nframe = 3;
    %[sgplevs]    = sgolayfilt(double(profX.plevs(xnlevs,ii)') ,npoly,nframe);
    %[sgwatercld] = sgolayfilt(double(profX.clwc(xnlevs,ii)')  ,npoly,nframe);
    %[sgicecld]   = sgolayfilt(double(profX.ciwc(xnlevs,ii)')  ,npoly,nframe);
  else
    % few points
    plevs    = profX.plevs(xnlevs,ii);
    watercld = profX.clwc(xnlevs,ii);
    icecld   = profX.ciwc(xnlevs,ii);
  end
...
%{
  ...
  if iiiiA == 1833
    ...
    plot(profX.plevs(xnlevs,ii),profX.clwc(xnlevs,ii),'bo-',plevs,watercld,'r-',plevs([wT wB]),[1 1]*wPeak,'kx-')
    keyboard
  end
%}
end    %% loop over iiiiA
