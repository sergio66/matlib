...
% junky cloud stuff
if (isfield(profin,'watercldX'))
   prof.watercldX=profin.watercldX(indp);
end
if (isfield(profin,'icecldX'))
   prof.icecldX=profin.icecldX(indp);
end
if (isfield(profin,'watercldY'))
   prof.watercldY=profin.watercldY(indp);
end
if (isfield(profin,'icecldY'))
   prof.icecldY=profin.icecldY(indp);
end
...
if length(fieldsIN) ~= length(fieldsOUT)
  ...
  if length(fieldsIN) > length(fieldsOUT)
    for ii = 1 : length(fieldsIN)
      finName = fieldsIN{ii};
      iaFound(ii) = -1;
      for jj = 1 : length(fieldsOUT)
        foutName = fieldsOUT{jj};
