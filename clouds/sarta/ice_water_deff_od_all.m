...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  %plot(cld_de_ice,1:91); set(gca,'ydir','reverse'); 
  %title([num2str(ii) ' : ' num2str(mean(cld_de_ice))]); pause(0.1)
...
junk = p1.sarta_lvlDMEwater; junk(junk < eps) = nan;
figure(1); scatter_coast(p1.rlon,p1.rlat,10,nanmean(junk));; title('mean DME water')
  caxis([15 25]); colorbar

junk = p1.sarta_lvlODwater;
figure(2); scatter_coast(p1.rlon,p1.rlat,10,nanmean(junk));; title('mean OD water')
  caxis([0 1]); colorbar

junk = p1.sarta_lvlDMEice; junk(junk < eps) = nan;
figure(3); scatter_coast(p1.rlon,p1.rlat,10,nanmean(junk));; title('mean DME ice')
  caxis([75 85]); colorbar

junk = p1.sarta_lvlODice; junk(junk < eps) = nan;
figure(4); scatter_coast(p1.rlon,p1.rlat,10,nanmean(junk));; title('mean OD ice')
  caxis([0 1]); colorbar
%}
